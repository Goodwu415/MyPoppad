...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_CREATE:
			...
			//文本框 最多能包含的字符的个数为32000
			SendMessage(hwndEdit, EM_LIMITTEXT, 32000, 0L);
			PopFIleInitilaize(hwnd);
			PopFontInitialize(hwndEdit);
			//定义一个新的窗口消息，保证该消息在系统范围内是唯一的。
		...
		case WM_INITMENUPOPUP:
			...
			return 0;
		case WM_COMMAND://LOWORD(wParam):子窗口id，lParam: 子窗口句柄
			...
			{//来自文件菜单的消息
				...
				case IDM_FILE_OPEN:
					if(bNeedSave && IDCANCEL == AskAboutSave(hwnd, szTitleName))
							return 0;
					if(PopFileOpenDlg(hwnd, szFileName, szTitleName))//打开对话框成功
					{
						if(!PopFileRead(hwndEdit, szFileName))//读取文件失败
						{
...
BOOL CALLBACK AboutDlgProc(HWND hDlg, UINT messge, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		case WM_INITDIALOG:
